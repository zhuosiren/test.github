    ...
    , ui(new Ui::doctorview)
{
    ...
    connect(ui->btSortAscending, &QPushButton::clicked, this, &doctorview::on_btSortAscending_clicked);
    connect(ui->btSortAscending2, &QPushButton::clicked, this, &doctorview::on_btSortAscending2_clicked);
}
...
// 实现升序排序的槽函数
// 实现升序排序的槽函数
void doctorview::on_btSortAscending_clicked()
{
    IDatabase &iDatabase = IDatabase::getInstance();
    // 获取数据模型（这里假设名为DoctorTabModel，根据实际情况调整）
    QAbstractItemModel *model = iDatabase.DoctorTabModel;
    if (model)
    {
...
void doctorview::on_btSortAscending2_clicked()
{
    IDatabase &iDatabase = IDatabase::getInstance();
    // 获取数据模型（这里假设名为DoctorTabModel，根据实际情况调整）
    QAbstractItemModel *model = iDatabase.DoctorTabModel;
...
void doctorview::on_btExport_clicked()
{
    ...
    QString filePath = QFileDialog::getSaveFileName(this, tr("保存医生数据文件"), "", tr("文本文件 (*.txt);;CSV 文件 (*.csv)"));
    if (!filePath.isEmpty())
    {
        // 调用导出数据函数
        if (exportDoctorData(filePath))
        {
            qDebug() << "医生数据导出成功";
        }
        else
...
bool doctorview::exportDoctorData(const QString& filePath)
{
    QFile file(filePath);
...
{
    QString filePath = QFileDialog::getOpenFileName(this, tr("选择医生数据文件"), "", tr("文本文件 (*.txt);;CSV文件 (*.csv)"));
    if (!filePath.isEmpty())
    {
        if (importDoctorData(filePath))
        {
            qDebug() << "医生数据导入成功";
        }
        else
...
bool doctorview::importDoctorData(const QString& filePath)
{
    QFile file(filePath);
    ...
    // 检查字段名是否符合要求
    QStringList requiredFields = QStringList() << "ID" << "AGE" << "NAME" << "SEX" << "DEPARTMENT_ID";
